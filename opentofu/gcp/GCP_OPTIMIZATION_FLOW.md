...
### Problems
- SA key live 4 places (disk / bucket / yaml / rclone)
- `roles/storage.admin` project scope — over-broad
- Public buckets get `objectAdmin@allUsers` — anyone write
- Storage module mandatory — no reuse existing buckets
- GH secret hold static JSON key — leak = full compromise
...
2. WIF provider `attribute-condition` pin to repo — other repos cannot impersonate.
3. Deploy SA bound via IAM condition `resource.name == cluster` — scoped one cluster.
4. Public buckets read-only for `allUsers`.
5. If future Tofu PR reintroduce `google_service_account_key`, apply fail with `permissionDenied`.