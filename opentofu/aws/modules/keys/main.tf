...
resource "null_resource" "upload_global_jwt_values_yaml" {
  ...
  provisioner "local-exec" {
      command = "aws s3 cp ${var.base_location}/../../../../scripts/global-values-jwt-tokens.yaml s3://${var.storage_container_private}/${var.environment}-global-values-jwt-tokens.yaml"
  }
...
resource "null_resource" "upload_global_rsa_values_yaml" {
  ...
  provisioner "local-exec" {
      command = "aws s3 cp ${var.base_location}/../../../../scripts/global-values-rsa-keys.yaml s3://${var.storage_container_private}/${var.environment}-global-values-rsa-keys.yaml"
  }
