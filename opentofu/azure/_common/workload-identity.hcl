...
inputs = {
  ...
  storage_account_id                 = local.skip_storage_module ? "/subscriptions/${local.subscription_id}/resourceGroups/${get_env("AZURE_OPENTOFU_BACKEND_RG")}/providers/Microsoft.Storage/storageAccounts/${local.cloud_vars.global.cloud_storage_access_key}" : dependency.storage.outputs.azurerm_storage_account_resource_id
